    ...
    for attack_message in attack_messages:
        ...
        if type(next_opposite_first_message_chance) == TeamMessage and next_opposite_first_message_chance.success_failure == Message.FAILURE:
            message_attack_method = AttackMethodPlayer.COURT_IN
                ...
                next_opposite_first_message_chance) == BlockMessage and next_opposite_first_message_chance.success_failure == Message.FAILURE:
            message_attack_method = AttackMethodPlayer.BLOCK_OUT
        elif type(
                next_opposite_first_message_chance) == DigMessage and next_opposite_first_message_chance.success_failure == Message.FAILURE:
        ...
        attack_method_eff.append_effect_calc(Messages([attack_message]), game, "공격종합", AttackMethodPlayer,
                                             message_attack_method, receive_result)
...
for i, one_team in enumerate(women_teams_before_2021_2022):
    wb.create_sheet(one_team, index=i)
    sheet = wb[one_team]

    court_in_eff = EffCalc(
        filter(lambda item: item[0].attack_method == AttackMethodPlayer.COURT_IN and item[0].team == one_team
               , attack_method_eff.items()))
    block_out_eff = EffCalc(
        filter(lambda item: item[0].attack_method == AttackMethodPlayer.BLOCK_OUT and item[0].team == one_team
               , attack_method_eff.items()))
    dig_fault_eff = EffCalc(
        filter(lambda item: item[0].attack_method == AttackMethodPlayer.DIG_FAULT and item[0].team == one_team,
               attack_method_eff.items()))
