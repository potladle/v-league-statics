...
def extract_text_from_tags(iterator: Iterator) -> List[str]:
...
def find_opposite_num_in_volleyball(position_num: int):
    if position_num > 3:
        return position_num - 3
    else:
        return position_num + 1


def select_last_of_list(object_list: list):
    return object_list[len(object_list) - 1]
