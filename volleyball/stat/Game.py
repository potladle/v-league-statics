...
def make_team_scores(team_scores):
    name_and_scores: List = team_scores.findAll("td")
...
def make_players_stats(players_stats_tag):
...
def make_game(soup: BeautifulSoup, link: str, index: int) -> Game:
    left_team_scores = soup.select("#wrp_popup > div.wrp_live_top > article > table > tbody > tr > td:nth-child(3) > table > tbody > tr:nth-child(1)")[0]
    right_team_scores = soup.select("#wrp_popup > div.wrp_live_top > article > table > tbody > tr > td:nth-child(3) > table > tbody > tr:nth-child(2)")[0]
    ...
    left_players_stats_tag = soup.select("#wrp_popup > div.wrp_live_bottom > div:nth-child(1) > div > table > tbody")[0]
    right_players_stats_tag = soup.select("#wrp_popup > div.wrp_live_bottom > div:nth-child(3) > div > table > tbody")[0]
    ...
    return Game(season, [left_team, right_team], [left_final, right_final], [left_scores, right_scores]
                , [left_players_stats, right_players_stats], game_num, index)
