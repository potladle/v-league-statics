...
class Rallies(List[Rally]):
    ...
    def select_rallies_in_message_index_range(self, indices: List[int]):
        rallies = self.__class__([])
        for rally in self:
            if rally.message_index[0] >= indices[0] and rally.message_index[1] <= indices[1]:
