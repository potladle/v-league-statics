...
def make_rallies(messages: List[Messages], set_for_rallies: Set, game: Game) -> Rallies:
    index = 0
    rallies: List[Rally] = []
        ...
        try:
            ...
            if type(left_message) == SubstituteOutMessage:
                ...
                rotation[1][idx] = next_right_message.name
                    ...
                    and ((issubclass(type(next_left_message), ServeMessage) or issubclass(type(next_right_message),
                                                                                          ServeMessage))
                         or (
                                 next_left_message.success_failure == Message.ORDINARY and next_right_message.success_failure == Message.ORDINARY)):
    ...
    rallies[len(rallies) - 1].winner = (lambda one_score: Rally.LEFT if one_score[0] > one_score[1] else Rally.RIGHT)(
        [game.scores[0][set_for_rallies.index], game.scores[1][set_for_rallies.index]])
