...
from volleyball.stat.Set import make_set, Set
from volleyball.stat.make_rallies import make_rallies, Rallies, Rally
    ...
    def fill_rallies(self):
        ...
        for i, game_for_rally in alive_it(enumerate(self.sets), len(self.sets)):
            ...
            for k, volleyball_set_for_rally in enumerate(game_for_rally):
                try:
                    self.rallies[i].append(make_rallies(self.messages[i][k], volleyball_set_for_rally, self.games[i]))
                except (IndexError, ValueError) as _:
...
class StatFile:
    ...
    def process_in_set_messages_by_messages(self, method, *args):
        print("process_in_set")
        for game in alive_it(self.games, len(self.games)):
            i = 0
            while True:
                try:
                    for set_in_game in self.sets[game.index - i]:
                        method(self.messages[game.index - i][set_in_game.index][0],
                               self.chances[game.index - i][set_in_game.index],
                               self.rallies[game.index - i][set_in_game.index], game, set_in_game, Rally.LEFT, *args)
                        method(self.messages[game.index - i][set_in_game.index][1],
                               self.chances[game.index - i][set_in_game.index],
                               self.rallies[game.index - i][set_in_game.index], game, set_in_game, Rally.RIGHT, *args)
                    break
                except IndexError:
                    print(f"error {i}")
                    print(traceback.format_exc())
...
class EffCalc(Dict[Player, List[int]]):
    ...
    def __append_directly(self, game: Game, message: Message, player_type, *args):
        player = message.make_player(game, player_type, *args)
    ...
    def append_effect_calc(self, messages: Messages, game: Game, action_type: str, player_type=Player, *args):
        ...
        for action_message in action_messages:
            ...
            self.__append_directly(game, action_message, player_type, *args)
    ...
    def return_whole_eff(self):
        whole_trial = 0
        whole_success = 0
        whole_failure = 0
        for value in self.values():
            whole_trial += value[0]
            whole_success += value[1]
            whole_failure += value[2]

        return [whole_trial, whole_success, whole_failure]
    ...
    def print_overall_effect(self, print_or_not: bool = True, prefix: str = "", at_least: int = 0):
        if self.return_whole_trial() >= at_least:
            overall_trial = 0
