...
class Messages(List[Message]):
    ...
    def find_set_of_setters(self, game, rallies, side):
        set_messages = self.__class__([])
        main_setter = self.find_main_player("세트", game)
        set_messages += self.select_messages_with_player_and_action(main_setter, "세트", game)

        substituted_players = list(dict.fromkeys(list(map(lambda one_message: one_message.make_player(game), self.select_messages_with_player_and_action(main_setter, "교체&투입", game)))))
        print(list(map(lambda player: player.name, substituted_players)))
        for substituted_player in substituted_players:
            attack_substituted_messages = self.select_messages_with_player_and_action(substituted_player, "공격종합", game)
            set_substituted_messages = self.select_messages_with_player_and_action(substituted_player, "세트", game)

            if len(set_substituted_messages) >= len(attack_substituted_messages):
                set_messages += self.select_messages_with_player_and_action(substituted_player, "세트", game)
            else:
                print(1)
                substitute_in_messages = self.select_messages_with_player_and_action(substituted_player, "투입", game)
                rally = rallies.select_rally_after_message_index(substitute_in_messages[0].index)

                opposite_player_name = rallies[rallies.index(rally) + 1].rotation[side][find_opposite_num_in_volleyball(rally.rotation[0].index(substituted_player.name))]
                set_messages += self.select_messages_with_player_name_and_action(opposite_player_name, "세트", game)
