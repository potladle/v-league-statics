...
class Message:
    ...
    def make_player(self, game: Game, player_type=Player, *args) -> Player:
        left_roster = list(map((lambda dictionary: list(dictionary.keys())[0]), game.stats[0]))
        if args != ():
            return player_type(self.name, self.back_num, game.season, (lambda name: game.teams[0] if name in left_roster else game.teams[1])(self.name), *args)
        else:
            return player_type(self.name, self.back_num, game.season, (lambda name: game.teams[0] if name in left_roster else game.teams[1])(self.name))
