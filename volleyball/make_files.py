...
from volleyball.utills import put_key_and_value_to_dictionary, take_key_by_value
...
def processor_in_set(messages: List[Messages], chances: Chances, rallies: Rallies, game: Game, set_in_game: Set):
    try:
        if game.index not in except_indices:
            ...
            if set_index == 0:
                os.makedirs(
                    f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})")
                json_file = json.dumps(game.__dict__, indent=4, ensure_ascii=False)
                file = open(
                    f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})\\game.txt "
                    , "w+"
                    , encoding="UTF-8")
                file.write(json_file)
                file.close()
            os.makedirs(
                f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})\\{set_index + 1}세트")

            json_file = json.dumps([list(put_key_and_value_to_dictionary(message.__dict__, "action",
                                                                         take_key_by_value(action_to_class,
                                                                                           type(message))) for message
                                         in messages[0])
                                       , list(put_key_and_value_to_dictionary(message.__dict__, "action",
                                                                              take_key_by_value(action_to_class,
                                                                                                type(message))) for
                                              message in messages[1])], indent=4, ensure_ascii=False)
            file = open(
                f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})\\{set_index + 1}세트\\messages.txt"
                , "w+"
                , encoding="UTF-8")
            file.write(json_file)
            file.close()
            ...
            file = open(
                f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})\\{set_index + 1}세트\\chances.txt"
                , "w+"
                , encoding="UTF-8")
            file.write(json_file)
            file.close()
            ...
            file = open(
                f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})\\{set_index + 1}세트\\rallies.txt"
                , "w+"
                , encoding="UTF-8")
            file.write(json_file)
            file.close()
            ...
            file = open(
                f"C:\\Users\\user\\Documents\\배구\\여자부\\20{season + 3}-20{season + 4} 시즌\\제 {game_num}경기 ({game.teams[0]},{game.teams[1]})\\{set_index + 1}세트\\set.txt"
                , "w+"
                , encoding="UTF-8")
            file.write(json_file)
            file.close()
